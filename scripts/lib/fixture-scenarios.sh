...
# FIXTURE_CMD_T_REPO, FIXTURE_TRIGGER_CMD_T, FIXTURE_PINNED, FIXTURE_ARCHIVED, and
...
fixture_scenario_names() {
    printf '%s\n' phase-1-release m6-status-sliver attention-only restore-banner orphan-banner sidebar-recent sidebar-pinned sidebar-pinned-alphabetical sidebar-archived file-tree-unavailable file-tree-permission cmd-t-repo-overview cmd-t-repo-terminal clean
}
