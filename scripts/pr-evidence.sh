...
# A test summary is text and goes up as text. Drawing it into an image was a
# way of satisfying a gate that accepted images, which Michael retired in
# 2026-09: "We will never again choose to create an svg of text just to have
# evidence. That was a reward hack I allowed to go through for a while."
write_summary_log() {
    local output="$1"
    local title="$2"
    shift 2
    ...
    {
        echo "$title"
        echo "Generated by scripts/pr-evidence.sh profile=$PROFILE"
        echo ""
        local line
        for line in "$@"; do
            echo "$line"
...
profile_swift_unit() {
    ...
    local summary_path="$RUN_DIR/$NAME.txt"
    write_summary_log "$summary_path" "PR #$PR Swift Unit Evidence" \
    ...
    upload_artifact "$NAME" "$summary_path"
    log "Paste the command and its result line into the PR body:"
    log "  \`swift test --filter $FILTER\` -- $focused_summary"
}
...
profile_ghostty_shortcuts() {
    ...
    local summary_path="$RUN_DIR/$NAME-log.txt"
    write_summary_log "$summary_path" "PR #$PR Ghostty Shortcut Smoke" \
    ...
    upload_artifact "$NAME-log" "$summary_path"
...
profile_performance() {
    ...
    local summary_path="$RUN_DIR/$NAME.txt"
    write_summary_log "$summary_path" "PR #$PR Performance Evidence" "${summary_lines[@]}" \
        "NOTE: PR markdown stored in $markdown_path"

    upload_artifact "$NAME" "$summary_path"
    log "Performance PR markdown: $markdown_path"
    log "The numbers are the evidence: paste them into the PR body's Performance section."
}
