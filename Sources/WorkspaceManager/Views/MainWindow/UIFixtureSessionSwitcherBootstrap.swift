...
/// Launch-surface request for the session-switcher fixture. The configuration type stays in
/// every build because the launch-surface wiring names it; only the environment parser is
/// debug-only, so release binaries carry neither the arming keys nor a way to arm them.
struct UIFixtureSessionSwitcherBootstrapConfiguration: Equatable, Sendable {
    #if DEBUG
        static let flagKey = "WORKSPACES_UI_FIXTURE_OPEN_SESSION_SWITCHER"
