...
Swift/SwiftUI code for the native macOS app; root `AGENTS.md` carries repo-wide process. `CLAUDE.md` here symlinks to this file — read one, not both.
...
5. Capture without forcing activation: `./scripts/capture-window.sh` (capture-only; with `--no-activate`, pause your own input during capture). Activation-driving scripts only when foreground input is acceptable; otherwise Tart/Lume or a separate macOS user/session.
...
6. **App Activation Policy**: All `NSApp.activate` calls route through `AppActivationPolicy` (`Sources/WorkspaceManager/App/AppActivationPolicy.swift`). Normal launch: full foreground; `WORKSPACES_NO_ACTIVATE_ON_LAUNCH=1`: dock visible, never steals focus (runtime too — the env var name is historical); `CI=true`: invisible accessory app. Headless launch scripts must set `WORKSPACES_NO_ACTIVATE_ON_LAUNCH=1`.
