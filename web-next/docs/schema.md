...
| `has_unpushed_work` | integer boolean | PR-from-session gate (#820, migration `0011_session_pull_requests`). The Vercel checkpoint tail sets this from `git rev-list` after committing local checkpoint work; the Open PR route clears it only after the sandbox push + draft PR command succeeds. |
