...
- Runtime `named_ci_evidence` maps each CI requirement to its exact head and latest run, including named non-required checks. A `satisfied` item needs no duplicate PR-body citation and can satisfy the review contract. `failed`, `pending`, `missing`, or `unavailable` cannot justify approval; name the specific live gap. This exception covers only a single whole named-CI requirement. Split combined checks and extra test obligations into separate requested items, with explicit evidence for each non-CI requirement; one green check never satisfies the whole compound item. Preserve non-CI and explicit human requirements.
