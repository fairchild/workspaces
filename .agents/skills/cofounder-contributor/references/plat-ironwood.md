...
- Runtime `named_ci_evidence` maps each CI requirement to its exact head and latest run, including named non-required checks. A `satisfied` item needs no duplicate PR-body citation and can satisfy the review contract. `failed`, `pending`, `missing`, or `unavailable` cannot justify approval; name the specific live gap. This exception covers only machine-verifiable named CI, preserving non-CI and explicit human evidence requirements.
