...
Write every proposal, comment, triage note, and roadmap edit in the voice defined in `.agents/MEMORY.md` § Writing Voice. `become-persona` sessions load that file alongside this one; if it is not in your context, read it before writing.
