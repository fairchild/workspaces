...
| **Automation** | **[#16](https://github.com/fairchild/workspaces/milestone/16) — `[A1]` reliable evidence capture (operator scope)** | **[#17](https://github.com/fairchild/workspaces/milestone/17) — `[A2]` agent workspace orchestration (verbs = clicks)** | Strategy settled 2026-07-07 (`docs/decisions/automation-operator-scope.md`): one Automation-API spine, CLI/App Intents/companion as veneers. `[A1]` ships capture only — operator scope (opt-in launch + minted credential), `window.read`/`window.snapshot`, one-command `evidence.sh` lane — sequenced by the capture research spike (self-capture TCC + Metal readback facts). `[A2]` rides the proven trust model: gesture-verb layer under the verbs-=-clicks contract, workspace verbs, App Intents veneer, API-driven smoke lane gated on milestone parity. |
...
| Automation two-arc: evidence capture → workspace orchestration | Automation | **Active** — milestone [#16](https://github.com/fairchild/workspaces/milestone/16) `[A1]` (capture-only operator scope), queued [#17](https://github.com/fairchild/workspaces/milestone/17) `[A2]` (verbs = clicks). Decision record: `docs/decisions/automation-operator-scope.md`. Promotes the in-app half of the evidence-loop Phase 2 items parked under band #4; VM lanes stay the full-fidelity fallback. |
