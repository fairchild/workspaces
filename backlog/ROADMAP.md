...
**Agent automation.** The v1 scheduled contributor fleet was retired on 2026-07-12. The Factory v2 pipeline is live as described in [the Agent Factory v2 plan](../docs/development/agent-factory-v2-plan.md).
...
| **web-next** | *(W6 complete — reusable Folio + MFWiki first-consumer arc approved, planning next)* | package seam → Workspaces consumer → private MFWiki cutover | [#19](https://github.com/fairchild/workspaces/milestone/19) `[W6]` **closed 2026-07-11**, all 12 issues closed: owned-host compute, approvals, local mode, steering, config parity, session workspace, native embedding, provider-aware validation, and harness cleanup. #998's broad remainder was made honest as #1044–#1046 rather than left as milestone fog. The next W-lane contract is the approved extraction of the Folio conversation experience as a reusable package, with Workspaces remaining its public source/consumer and MFWiki becoming the first external consumer. |
...
### 2026-07-11 — [W6] owned-host daily driver: 12 issues closed, validation became provider-shaped (#981–#1014)

- **The harness needed ownership, not another readiness wait.** A stale detached `next start` could satisfy the next run's health probe, so “server is ready” did not prove “this run owns the server.” PR [#1042](https://github.com/fairchild/workspaces/pull/1042) encoded the fix: occupied-port refusal, parent/process-group cleanup, partial-acquisition cleanup, and real bound-socket coverage. The same arc then exposed React replacement during screenshot scrolling; PR [#1043](https://github.com/fairchild/workspaces/pull/1043) moved all six sites behind a bounded detach-only retry that preserves Playwright strictness. Both failures are now machinery, not runbook advice.
- **A runtime check is only honest when it is provider-shaped.** Host compute was failing on an expired Vercel token it never used. PR [#1043](https://github.com/fairchild/workspaces/pull/1043) made preflight derive the same provider as session creation, fail closed for mock/unknown runtimes, gate host on Claude/git/owned-root writability, and run a real read-only host turn that proves durable completion + resume without weakening Vercel sandbox-leak semantics. Two targeted Fable passes caught the fail-open default, an incomplete read-only assertion, optional provider identity, and a duplicate-locator masking risk before merge.
- **Umbrella completion should separate the shipped invariant from product breadth.** #998's essential invariant—one persistent clone per host session—had shipped, while ownership manifests, retention/disk controls, and registered-checkout worktrees remained three different jobs. Closing the umbrella only after filing [#1044](https://github.com/fairchild/workspaces/issues/1044), [#1045](https://github.com/fairchild/workspaces/issues/1045), and [#1046](https://github.com/fairchild/workspaces/issues/1046) made W6 truthfully complete without losing the remainder.
- **Session retro: two avoidable rounds were policy drift, not hard engineering.** The first #1042 readiness run failed because the PR body used a near-miss label despite the canonical schema already being documented; no new encoding is warranted—the existing gate did its job, so this lesson is deliberately dropped after correcting operator practice. More materially, `AGENTS.md` requires a repo-owned `codex-review-loop` skill that does not exist; [#1047](https://github.com/fairchild/workspaces/issues/1047) now tracks restoring/replacing it plus a path-drift check. Until that closes, every PR must state which review actually ran rather than implying compliance.
