...
- **Never use bare `self-hosted` for workflows in this repo.** Use GitHub-hosted macOS (`macos-15`) for generic build/test jobs, `[self-hosted, lume-macos]` for maintained macOS automation and agent execution (with ubuntu-latest fallback when the workflow supports it), and `[self-hosted, signing-host]` for release/signing/notarization. The old Tart UI lane is retired.
