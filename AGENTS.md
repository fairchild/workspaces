...
| Spawn-scaled budget | `Helpers/SpawnBudget.swift` | Any deadline in a test that launches child processes |
...
- Wait on the observable state change, not on a tuned wall clock. Process-spawn latency spans two orders of magnitude between a laptop and a loaded hosted runner (0.1s vs 13-15s measured), so any fixed budget is a bet against it — scale from `SpawnBudget` instead. Where a real-time bound genuinely *is* the property (an OS-imposed ceiling), gate that one test with a named reason rather than stretching the number.
