...
- `scripts/factory-worker-identity.sh` — `source` this in a worker session; when `FACTORY_WORKER_IDENTITY=app` it mints a token, exports `GH_TOKEN`, and points local (`--local`, never global) git commit identity + a host-scoped credential helper at `workspaces-factory[bot]`. **Unset (the default) or any other value: verified no-op** — no env mutation, no git config change.
