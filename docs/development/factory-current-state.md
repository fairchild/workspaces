...
| Review Response | `factory-review-response.yml` | `pull_request_review` submitted as `changes_requested` on a same-repository head; or owner `workflow_dispatch` | `AGENT_AUTOMATIONS_ENABLED` + `FACTORY_REVIEW_RESPONSE_ENABLED`, both branches (dispatch does not bypass here) | **Dark** — merged with #1378; set `FACTORY_REVIEW_RESPONSE_ENABLED=true` to arm it. Answers a blocking review with either a self-clearing note or an explicit, gesture-named owner escalation; deterministic, no model. |
