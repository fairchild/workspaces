...
| `WORKSPACES_UI_FIXTURE_PINNED` | optional | Comma-separated workspace names, pinned in listed order so the sidebar's Pinned section renders. Debug-only (`UIFixtureSeeder.seedPinnedWorkspacesIfNeeded`), enforced absent from release by `scripts/check-release-harness-absence.sh`. The `sidebar-pinned` and `sidebar-pinned-alphabetical` scenarios set it. |
| `WORKSPACES_UI_FIXTURE_ARCHIVED` | optional | Comma-separated workspace names, archived in place so each owning repo renders its archived disclosure pill. Applied after the pins, so archiving unpins as it does in the app. Debug-only (`UIFixtureSeeder.seedArchivedWorkspacesIfNeeded`), enforced absent from release by `scripts/check-release-harness-absence.sh`. The `sidebar-archived` scenario sets it. |
| `WORKSPACES_UI_FIXTURE_SELECTED` | optional | One workspace name, selected at launch through the production selection path — the same call a click on its row makes, so the terminal attaches and the sidebar renders the active card. Applied after the surface lifecycle, so it wins over the restored or fallback surface; an archived or unknown name is logged and skipped. Debug-only (`UIFixtureSelectedWorkspaceBootstrapConfiguration`), enforced absent from release by `scripts/check-release-harness-absence.sh`. The `sidebar-active-card` scenario sets it. |
...
| `WORKSPACES_UI_FIXTURE_SEED_RUNAWAY_ALERT` | optional | `1` stages the sidebar's runaway-process alert strip with one deterministic synthetic alert (`UIFixtureRuntimeAlertBootstrap`, debug-only behind `#if DEBUG` per #1235/#1237 — the release build carries a stub and `scripts/check-release-harness-absence.sh` enforces the key's absence). Fixture mode always replaces the live process sweep — with the synthetic alert when this is set, with nothing otherwise — so a genuine runaway on the dev machine never leaks into a capture (issue #1368). The `runaway-alert` scenario sets it. |
