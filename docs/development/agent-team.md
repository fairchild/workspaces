...
- **Explicit workflow tokens** — the repository default `GITHUB_TOKEN` permission is read-only. Agent workflows that need writes declare them explicitly, and Peter Planner uses the `WORKSPACE_AGENTS_APP_ID` / `WORKSPACE_AGENTS_PRIVATE_KEY` GitHub App token for discussion and issue writes instead of relying on the repository default.
